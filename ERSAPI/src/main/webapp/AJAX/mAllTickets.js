...
function getAllTickets() {
    ...
    let reqURL = "http://localhost:8080/ERSAPI/api/manager/ticket";
...
function approveTicket() {
    ...
    let resultComment = document.getElementById("resultComment").value;
    if(array.length === 0) {
        window.alert("No ticket selected");
    } else {
        for(let tId of array) {
            reqURL = "http://localhost:8080/ERSAPI/api/manager/ticketapprove?ticketId=" + tId + "&statusId=3&comment=" + resultComment;
            let xhttp = new XMLHttpRequest();
...
function rejectTicket() {
    ...
    if(array.length === 0) {
        ...
        for(let tId of array) {
            reqURL = "http://localhost:8080/ERSAPI/api/manager/ticketapprove?ticketId=" + tId + "&statusId=4&comment=" + resultComment;
            let xhttp = new XMLHttpRequest();
            xhttp.open("PUT",reqURL);
            xhttp.send();
        }
